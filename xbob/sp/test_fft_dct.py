...
def _dct1D(N, t, eps):
  ...
  for i in range(N):
    assert compare(u_dct_idct[i], t[i], 1e-3)

  v_dct_idct = idct(dct(t))
  # get answer and compare to original
  for i in range(N):
    assert compare(v_dct_idct[i], t[i], 1e-3)

def _dct2D(M, N, t, eps):
  # process using DCT
  ...
  for i in range(M):
    for j in range(N):
      assert compare(u_dct_idct[i,j], t[i,j], 1e-3)
...
def _fft1D(N, t, eps):
  ...
  # get answer and compare to original
  for i in range(N):
    assert compare(u_fft_ifft[i], t[i], 1e-3)

  v_fft_ifft = ifft(fft(t))
  # get answer and compare to original
  for i in range(N):
    assert compare(v_fft_ifft[i], t[i], 1e-3)

def _fft2D(M, N, t, eps):
  # process using FFT
  ...
  for i in range(M):
    for j in range(N):
      assert compare(u_fft_ifft[i,j], t[i,j], 1e-3)
...
def test_dct1D_range1to2048_random():
...
def test_dct2D_range1x1to64x64_random():
...
def test_fft1D_range1to2048_random():
...
def test_fft2D_range1x1to64x64_random():
